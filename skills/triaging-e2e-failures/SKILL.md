...
| 步骤 | 执行者 | 工具/参考 | 输出 |
|------|--------|-----------|------|
| **Step 1** 解析 | Python 脚本（确定性） | `scripts/parse_trace.py` | 结构化 JSON + 文本摘要 |
| **Step 2** 分类 | AI 推理 | `references/triage_rules.md` | 类别 + 置信度 + 关键证据 |
| **Step 3** 修复 | AI 推理 | `references/fix_patterns.md` | 可执行修复代码（仅 flaky_element） |
| **Step 4** 报告 | AI 生成 | `assets/report_template.md` | 标准化 Markdown 根因报告 |
...
**触发示例**：`"请分析测试结果，trace文件是 output/traces/2026-05-26-1/test_login.zip"`

#### Step 1：运行解析脚本（工具调用）

先定位 skill 自身目录（无论安装在项目还是个人目录），再调用入口脚本：

```bash
# 定位 skill 目录（兼容 .claude/skills、~/.claude/skills、自定义位置）
SKILL_DIR=$(find . ~/.claude -maxdepth 6 -name "run.sh" \
    -path "*/triaging-e2e-failures/*" 2>/dev/null | head -1 | xargs dirname)

# 输出文本摘要（供 AI Step 2/3/4 推理）
bash "$SKILL_DIR/run.sh" <trace路径> \
    --out output/reports/<name>_summary.txt

# 可选：输出结构化 JSON
bash "$SKILL_DIR/run.sh" <trace路径> --json \
    --out output/reports/<name>_trace.json

# 可选：包含 DOM 快照（元素类失败推荐）
bash "$SKILL_DIR/run.sh" <trace路径> --dom \
    --out output/reports/<name>_summary.txt
```

#### Step 2：读取摘要 → 参照 `references/triage_rules.md` 分类

读取上一步生成的摘要，按 `triage_rules.md` 规则判断：
...
参照 `references/fix_patterns.md`，结合：
...
严格遵循模板结构，不省略任何一级标题，根据 category 只填写对应的"修复建议"分支。
...
├── skills/triaging-e2e-failures/   ← 本 Skill 目录
├── utils/
...
- trace.zip 由测试失败时自动生成，路径格式：`output/traces/<YYYY-MM-DD-序号>/<test_name>.zip`。
...
- 编程调用示例：
  ...
  print(summary)  # 将 summary 发送给 AI 接口，或在对话中粘贴
  ```
