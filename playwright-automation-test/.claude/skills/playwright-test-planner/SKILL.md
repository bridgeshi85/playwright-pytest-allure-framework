...
- `test_focus`：用户原话中明确提到的测试范围（如"登录功能"、"搜索筛选"、"删除后的确认弹窗"）；若用户仅提供 URL 未描述范围，则为空，由下方作用域推断规则决定覆盖深度
...
根据 `test_focus` 和 Step 2 提取的页面结构，列出所有**值得测试**的场景候选。按以下维度扩展，有依据才提，不随意堆砌：
...
若多个场景满足以下全部条件，**合并为一个 `type: parametrize` 场景**，不分开列出：
1. **类型相同**：正向归正向，负向归负向，**不允许跨类型合并**
2. 操作流程完全相同（相同的页面、相同的步骤顺序）
3. 仅输入数据不同（字段值、为空、格式非法、不同账号等变体）
4. 断言预期相同（如同样"跳转成功"、同样"停留在当前页"）

典型例子：
- **正向**：「管理员登录」「普通用户登录」「只读用户登录」→ 合并为 `TC_LOGIN_VALID` parametrize（各角色数据放 JSON）
- **负向**：「密码错误」「用户名为空」「密码为空」→ 合并为 `TC_LOGIN_INVALID` parametrize（各无效输入放 JSON）
- **不合并**：正向成功登录 + 负向登录失败 → 保持独立，类型不同不得混入同一参数化
...
|--------|------|------|---------|
| TC_XXX_001 | … | positive | … |
| TC_XXX_002 | … | parametrize | 多组输入验证同一失败路径 |
| TC_XXX_003 | … | negative | 独立异常场景（流程不同，不可合并） |

**至少包含 1 个正向场景和 1 个负向/参数化场景**，其余根据页面实际特征决定。
